...
-- Enum per livelli dei corsi (skip if exists)
DO $$ BEGIN
...
-- Enum per tipi di lezione (skip if exists)
DO $$ BEGIN
...
-- Enum per livelli di rischio (skip if exists)
DO $$ BEGIN
...
-- ============================================================================
-- TRIGGER PER UPDATED_AT (skip if exists)
-- ============================================================================
DROP TRIGGER IF EXISTS update_educational_courses_updated_at ON public.educational_courses;
...
DROP TRIGGER IF EXISTS update_user_risk_profiles_updated_at ON public.user_risk_profiles;
...
DROP POLICY IF EXISTS "Anyone can view published courses" ON public.educational_courses;
CREATE POLICY "Anyone can view published courses"
  ON public.educational_courses FOR SELECT
  USING (is_published = true);

DROP POLICY IF EXISTS "Admins can manage all courses" ON public.educational_courses;
...
DROP POLICY IF EXISTS "Users can insert their own risk profile" ON public.user_risk_profiles;
CREATE POLICY "Users can insert their own risk profile"
  ON public.user_risk_profiles FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own risk profile" ON public.user_risk_profiles;
CREATE POLICY "Users can update their own risk profile"
  ON public.user_risk_profiles FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all risk profiles" ON public.user_risk_profiles;
